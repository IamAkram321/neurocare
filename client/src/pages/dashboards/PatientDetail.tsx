...
import { AlertTriangle, Share2, Download, ArrowLeft, Plus, Trash2, CheckCircle2, Circle } from "lucide-react";
    ...
    <DashboardLayout>
       <div className="space-y-6">
         ...
         {patient.status === "critical" && (
           <div className="rounded-lg border-l-4 border-l-destructive bg-destructive/10 p-4 flex items-center">
             <AlertTriangle className="h-5 w-5 mr-3 text-destructive" />
             <p className="font-bold text-destructive">CRITICAL MONITORING ACTIVE</p>
           </div>
         )}
                    ...
                    <Card>
                        ...
                        {user.role === 'doctor' && (
                          <Dialog>
                            ...
                            <DialogContent>
                              ...
                              <DialogFooter><Button onClick={() => { setMeds([...meds, { id: Date.now(), ...newMed, status: "Active" }]); setNewMed({ name: "", dose: "" }); }}>Add</Button></DialogFooter>
                            </DialogContent>
                          </Dialog>
                        )}
                      </CardHeader>
                      <CardContent>
                        <ul className="space-y-3">
                          {meds.map(m => (
                            <li key={m.id} className="flex justify-between items-center p-3 rounded bg-muted/30 border border-border/50">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {user.role === 'doctor' && <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => setMeds(meds.filter(x => x.id !== m.id))}><Trash2 className="h-4 w-4" /></Button>}
                              </div>
                            </li>
                          ))}
                        </ul>
                      ...
                      <CardHeader className="flex flex-row items-center justify-between">
                        ...
                        {user.role === 'doctor' && (
                          <div className="flex gap-2">
                            ...
                            <Button size="sm" onClick={() => { if(newNote) { setNotes([{ id: Date.now(), author: user.name, time: "Just now", text: newNote }, ...notes]); setNewNote(""); } }}><Plus className="h-4 w-4" /></Button>
                          </div>
                        )}
                      </CardHeader>
                      <CardContent className="space-y-4">
                        {notes.map(n => (
                          <div key={n.id} className="group flex gap-4 p-3 rounded bg-muted/20 border border-border/40 relative">
                             ...
                             {user.role === 'doctor' && <Button variant="ghost" size="icon" className="h-7 w-7 opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => setNotes(notes.filter(x => x.id !== n.id))}><Trash2 className="h-4 w-4 text-destructive" /></Button>}
                          </div>
                        ))}
                      </CardContent>
                   </Card>
